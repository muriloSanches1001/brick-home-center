  ...
  <div class="w-full h-full fixed top-0 left-0 z-20 flex items-start bg-black/70">
    ...
    >
      ...
      <div class="flex flex-col gap-4 px-4 sm:px-10 py-6">
        <BaseHeading as="h3" size="2xl">Sobre a Brick</BaseHeading>
        <base-text size="sm" as="p">A Brick é uma empresa de construção focada em entregar obras seguras, bem-feitas e adaptadas às necessidades de cada cliente.</base-text>
        <ul class="flex flex-col gap-1">
          <li v-for="benefit in benefits" :key="benefit" class=" text-neutral-900 text-lg font-medium flex items-center gap-2">
            <Icon
              name="uil:check"
              class="w-5 h-5 text-slate-700"
              aria-hidden="true"
            />
            <base-text size="sm" as="span">{{ benefit }}</base-text>
          </li>
        </ul>
        <base-heading as="h4" size="lg">Acessibilidade</base-heading>
        <!--    TODO: Add accessibility section    -->
        <base-heading as="h4" size="lg">Social</base-heading>
        <ul class="flex gap-2">
